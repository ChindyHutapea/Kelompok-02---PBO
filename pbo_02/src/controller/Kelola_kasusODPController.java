 ...
 * @author ITD
 */
public class Kelola_kasusODPController implements Initializable {
    ...
    @FXML
    private TableView<ODP> tableODP;

    @FXML
    private TableColumn<ODP, String> col_id;

    @FXML
    private TableColumn<ODP, String> col_tanggal;

    @FXML
    private TableColumn<ODP, String> col_jumlah;
    ...
    ObservableList<ODP> observableList = FXCollections.observableArrayList(); 
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Preferences odpPreferences = Preferences.userRoot();
        String odp = odpPreferences.get("jumlah",null);
    ...
    void onClickButtonTambah(ActionEvent event) {
        ...
        api.createOdp(tanggal, jumlah).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if(response.isSuccessful()){
                    Platform.runLater(() -> {   
                    try{
                        FXMLLoader pindahPengguna = new FXMLLoader(getClass().getResource("/view/Kelola_kasusODP.fxml"));
         ...
         private void tampilODP() {
         Api api=RetrofitClient.getRetrofit().create(Api.class);
        
        Call<ODPResponse> call=api.allOdp();
        call.enqueue(new Callback<ODPResponse>(){
            @Override
            public void onResponse(Call<ODPResponse> call, Response<ODPResponse> response) {
                if(response.isSuccessful()){
                    int size=response.body().getODP().size();
                    List<ODP> listODP=response.body().getODP();
                    
                    for(int i=0; i<size; i++){
                      
                        observableList.add(new ODP(listODP.get(i).getId(), listODP.get(i).getTanggal(), listODP.get(i).getJumlah()));                         
                    }
                    ...
                    tableODP.setItems(observableList);
                }
                else{
                    System.out.println(response.message());
                }
            ...
            @Override
            public void onFailure(Call<ODPResponse> call, Throwable thrwbl) {
                throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
            }
    ...
    void onClickButtonUpdate(ActionEvent event) {
         if(tableODP.getSelectionModel().getSelectedItem()!=null){
           ODP listODP = tableODP.getSelectionModel().getSelectedItem();
            
            Preferences preferences = Preferences.userRoot(); 
            preferences.putInt("id",listODP.getId());
            preferences.put("tanggal",listODP.getTanggal());
            preferences.put("jumlah",listODP.getJumlah());
       
            
            try{
                FXMLLoader pindaEdit = new FXMLLoader(getClass().getResource("/view/EditODP.fxml"));
    ...
    void onClickButtonDelete(ActionEvent event) {
           if(tableODP.getSelectionModel().getSelectedItem()!= null){
            ODP preferences = tableODP.getSelectionModel().getSelectedItem();
            System.out.println(preferences.getJumlah());
            Api api = RetrofitClient.getRetrofit().create(Api.class);
            
            api.deleteOdp(preferences.getId()).enqueue(new Callback<ResponseBody>(){
                @Override
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    if(response.isSuccessful()){
                            Platform.runLater(() -> {
                                try{
                                    FXMLLoader pindahOdp = new FXMLLoader(getClass().getResource("/view/Kelola_kasusODP.fxml"));
                                    Parent parent1 = pindahOdp.load();
