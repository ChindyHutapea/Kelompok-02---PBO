 ...
 * @author gabri
 */
public class EditInformasiController implements Initializable {
    ...
    @FXML
    void onClickEditInformasi(ActionEvent event) {
     Api api = RetrofitClient.getRetrofit().create(Api.class);
           System.out.println(id);
           Preferences preferences = Preferences.userRoot(); 
    
            api.updateInfo(id, tf_tanggal.getText().toString(), tf_judul.getText().toString(), tf_isiInfo.getText().toString())
                .enqueue(new Callback<ResponseBody>(){
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if(response.isSuccessful()){
                            preferences.remove("id");
                            preferences.remove("judul");

                            Platform.runLater(() -> {   
                                try{
                                    FXMLLoader pinda2 = new FXMLLoader(getClass().getResource("/view/Kelola_Informasi.fxml"));
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       tf_judul.setText(preferences.get("judul", null));
       tf_isiInfo.setText(preferences.get("isi_info", null));
